...
app.get("/random", (req, res) => {
  const ranomIndex = Math.floor(Math.random() * jokes.length);
...
app.get("/filter", (req,res) => {
  ...
  const filteredActivites = jokes.filter((joke) => joke.jokeType === type);
  res.json(filteredActivites);
});
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const exsistingJoke = jokes.find((joke) => joke.id === id);
  const replacementJoke = {
    id: id,
    jokeText: req.body.text || exsistingJoke.text,
    jokeType: req.body.type || exsistingJoke.type,
  };
  const searchIndex = jokes.findIndex((joke) => joke.id == id);
  jokes[searchIndex] = replacementJoke;
...
app.delete("/all", (req, res) => {
  ...
  if(userKey === masterKey) {
    ...
    res
    .status(404)
    .json({ error: `ur not authorised to do this action`});
  }
});
